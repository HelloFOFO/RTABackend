        ...
        async.waterfall([
            ...
            function(cb){
                var opt = {
                    ...
                    path:"/ent/agent/member/list?page=0",
                    method:"GET"
                };
            ...
            function(r,cb){
                try{
                    var opt = {
                        hostname:config.inf.host,
                        port:config.inf.port,
                        path:"/ent/agent/shortList",
                        method:"GET"
                    };
                    new httpClient(opt).getReq(function(err,result){
                        if(result.error===0){
                            r.providerNames = result.data;
                           res.render('agentMemberManagement',r);
                        }
                        else{
                            cb(1,result);
                        }
...
exports.updatePMember = function(req,res){
    ...
    var opt = {
        hostname:config.inf.host,
        port:config.inf.port,
        path:"/ent/agent/member/update/"+req.params.id,
        method:"POST"
    };
    try{
...
exports.getProviderMember = function(req,res){
    var opt = {
        hostname:config.inf.host,
        port:config.inf.port,
        path:"/ent/agent/member/detail/"+req.body.id,
        method:"GET"
    };
    try{
        var http = new httpClient(opt);
...
exports.getProviderMembersList = function(req,res){
    ...
    var opt = {
        hostname:config.inf.host,
        port:config.inf.port,
        path:"/ent/agent/member/list?"+params,
        method:"GET"
    };
    console.log(params);
    var ret = {};
